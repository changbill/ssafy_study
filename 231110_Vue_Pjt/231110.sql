...
# 조회
# board와 video의 id가 일치하는 부분 조회
SELECT 
    * 
from board A INNER JOIN video B
ON A.video_id = B.video_id;

# video와 board를 video_id 컬럼을 기준으로 하여 inner join한 후 해당하는 video테이블의 video.video_id, video.title, youtube 조회
SELECT video.video_id, video.title, youtube  FROM video INNER JOIN board ON video.video_id = board.video_id;