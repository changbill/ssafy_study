...
public class MovieController {
	...
	public String showList(Model model) {
		//코드작성
		List<Movie> list = mService.selectAll();
	 ...
	 */
	@PostMapping("/regist")
    public String doRegist(@ModelAttribute Movie movie, MultipartFile file) throws IllegalStateException, IOException {
        // 변경된 파일 이름이 적용된 Movie를 MovieService를 통해 DB에 저장한다.
        // T.X 처리를 위해 파일 저장에 대한 부분을 service로 이동시킨다.
        // 코드자성

        if (file != null && file.getSize() != 0) {
            //classpath? 빈이 클래스를 만들어서.. 글로..저장이 된다나..
